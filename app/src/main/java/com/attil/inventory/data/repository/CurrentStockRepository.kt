    ...
    fun getCurrentStocksByCategory(categoryName: String): Flow<Result<List<CurrentStock>>> = flow {
        val result = try {
            ...
            if (response.isSuccessful) {
                ...
                Result.failure(Exception("Failed to filter by category: ${response.code()} - $errorBody"))
            }
        } catch (e: Exception) {
    ...
    fun getCurrentStocksByGodown(godownName: String): Flow<Result<List<CurrentStock>>> = flow {
        val result = try {
            ...
            if (response.isSuccessful) {
                ...
                Result.failure(Exception("Failed to filter by godown: ${response.code()} - $errorBody"))
            }
        } catch (e: Exception) {
    ...
    fun getLowStockItems(): Flow<Result<List<CurrentStock>>> = flow {
        val result = try {
            ...
            if (response.isSuccessful) {
                ...
                Result.failure(Exception("Failed to get low stock items: ${response.code()} - $errorBody"))
            }
        } catch (e: Exception) {
